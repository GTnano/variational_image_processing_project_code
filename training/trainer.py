...
class Trainer:
    ...
    def __init__(self, config, seed, device):
        ...
        self.noise_type = 'salt_and_pepper' # 'gaussian', 'salt_and_pepper', 'speckle'
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(tbar):
            ...
            if self.noise_type == 'speckle':
                noise = data * self.sigma/255 * torch.randn(data.shape,device=data.device)
                noisy_data = data + noise
            elif self.noise_type == 'salt_and_pepper':
                noisy_data = torch.clone(data)
                salt = np.random.rand(*list(noisy_data.shape)) < salt_ratio
                pepper = np.random.rand(*list(noisy_data.shape)) < pepper_ratio
                noisy_data[salt] = 1
                noisy_data[pepper] = 0
            else:
